...
Веб-калькулятор для расчета норм расхода электроэнергии в грузовом движении на тягу поездов. Калькулятор динамически загружает маршруты из папки `/data` и поддерживает пользовательские маршруты с индивидуальными коэффициентами.
...
Калькулятор является полноценным PWA и может быть установлен как приложение:
...
1. **Определение нагрузки на ось**:
   ```
   нагрузка_на_ось = вес_поезда / количество_осей
   ```

2. **Поиск коэффициента**:
   - Сначала программа ищет индивидуальные коэффициенты для конкретного маршрута в файле маршрута
   - Если такие коэффициенты существуют, используется ближайшее значение к вычисленной нагрузке на ось
   - Если индивидуальных коэффициентов нет, используется стандартный коэффициент для данного типа локомотива

3. **Округление нагрузки на ось**:
   - Нагрузка на ось округляется до ближайшего целого числа
   - Затем находится соответствующее значение коэффициента в таблице

4. **Интерполяция**:
   - Если точного совпадения нагрузки на ось нет в таблице, используется ближайшее значение
...
### Управление пользовательскими маршрутами
- **Редактор маршрутов** - Текстовый редактор для создания маршрутов в Markdown формате
- **Загрузка маршрутов** - Парсинг и добавление маршрутов из текста
- **Сохранение в localStorage** - Локальное сохранение пользовательских маршрутов
- **Экспорт маршрутов** - Сохранение пользовательских маршрутов в .md файл
- **Предложение маршрутов** - Отправка маршрутов разработчикам через email

### Форматы пользовательских маршрутов

1. **Полный формат** с индивидуальными коэффициентами:
```
# Москва - СПб
## 635 км
...
2. **Простой формат** со стандартными коэффициентами:
```
Москва - СПб | 635
Казань - НН | 400
```
...
Проект предназначен для развертывания на GitHub Pages как статический веб-сайт.
...
- **v2.0** - Динамическая загрузка маршрутов, улучшенное управление пользовательскими маршрутами
- **v1.5** - Добавлена поддержка пользовательских маршрутов и тем
- **v1.0** - Базовая функциональность калькулятора
